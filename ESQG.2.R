...
library(foreign); library(readstata13); library(lattice)
library(ggplot2); library(plyr); library(dplyr)
library(plotly); library(vcd); library(mosaic); library(lme4); library(xtable)
...
qog_var <- names(qog1) %in% c("ccode", "year", "fh_rol", "fh_polity2", "pwt_hci", "dpi_system", "ht_regtype", "p_polity", "p_polity2", "undp_hdi", "gle_cgdpc", "imf_gdpc", "imf_gdpcppp", "gle_rgdpc") 
qog2 <- qog1[qog_var]
...
mr1$soe <- NULL
mr1$soe[mr1$car4 >= 1] <- "Soe"
mr1$soe[mr1$car4 < 1] <- "Non Soe"
table(mr1$soe); mr1$soe
# plot(table(as.character(mr1$soe)))

mr1 %>% filter(!is.na(soe)) -> mr1soe

count_soe <- ggplot(mr1soe, aes(x = mr1soe$soe)) + 
#  geom_text(vjust = 1.5, colour = "white") +
...
experfobst1 <- mosaic( ~ es2$perf11 + es2$obst1 + es2$obst11, data = es2)
...
t(as.matrix(table(es2$obst1))); t(as.matrix(table(es2$obst2)))
t(as.matrix(table(es2$obst3))); t(as.matrix(table(es2$obst4)))
t(as.matrix(table(es2$obst5))); t(as.matrix(table(es2$obst6)))
t(as.matrix(table(es2$obst7))); t(as.matrix(table(es2$obst8)))
t(as.matrix(table(es2$obst9))); t(as.matrix(table(es2$obst10)))
t(as.matrix(table(es2$obst11))); t(as.matrix(table(es2$obst12)))
...
obsttable <- rbind(t(as.matrix(table(es2$obst1))), t(as.matrix(table(es2$obst2))), t(as.matrix(table(es2$obst3))), t(as.matrix(table(es2$obst4))), t(as.matrix(table(es2$obst5))), t(as.matrix(table(es2$obst6))),
                   t(as.matrix(table(es2$obst7))), t(as.matrix(table(es2$obst8))), t(as.matrix(table(es2$obst9))), t(as.matrix(table(es2$obst10))),
                   t(as.matrix(table(es2$obst11))), t(as.matrix(table(es2$obst12))), t(as.matrix(table(es2$obst13))),
                   t(as.matrix(table(es2$obst14))), t(as.matrix(table(es2$obst15))))
...
table(es2$obst9); table(es2$obst10); table(es2$obst11); table(es2$obst12)
table(es2$obst13); table(es2$obst14); table(es2$obst15)

####################################################################################################

table(es2$fin2); table(es2$stra_sector); table(es2$fh_rol)
es2$fh_rol2 <- es2$fh_rol + 1
table(es2$fh_rol2); table(es2$pwt_hci); table(es2$dpi_system)
table(es2$fh_polity2); table(es2$ht_regtype); table(es2$p_polity)
table(es2$p_polity2); table(es2$undp_hdi)

table(es2$gle_cgdpc, es2$year.x)
# table(es2$imf_gdpc, es2$year.x)
# table(es2$imf_gdpcppp)
table(es2$gle_rgdpc, es2$year.x)

ggplot(es2, aes(x = factor(es2$year.x), y = es2$gle_cgdpc)) + geom_boxplot()
ggplot(es2, aes(x = factor(es2$year.x), y = log(es2$gle_cgdpc))) + geom_boxplot()
# ggplot(es2, aes(x = factor(es2$year.x), y = es2$imf_gdpcppp)) + geom_boxplot()
ggplot(es2, aes(x = factor(es2$year.x), y = es2$gle_rgdpc)) + geom_boxplot()

summary(es2$gle_cgdpc)
summary(es2$imf_gdpc)
summary(es2$gle_rgdpc)
...
t(as.matrix(summary(es2$gle_cgdpc)))
statetable <- rbind(t(as.matrix(summary(es2$pwt_hci))), t(as.matrix(summary(es2$fh_rol2))), t(as.matrix(summary(es2$p_polity2))), t(as.matrix(summary(es2$gle_cgdpc))))
colnames(statetable) <- c("Min", "1st Qu.", "Median", "Mean", "3rd Qu.", "Max", "NA's")
rownames(statetable) <- c("Human Capital", "Rule of Law", "Polity", "GDP per Capita")
statetable
...
model_0_1 <- lm(perf1 ~ exporter + size + car1 + car6 +
...
model_0_2 <- lm(perf1 ~ pwt_hci + fh_rol2 + p_polity2 + log(gle_cgdpc), data = es2)
summary(model_0_2)

# options(digits = 4)
# model_0_3 <- lmer(perf1 ~ exporter + size + car1 + car6 + 
...
model_0_4 <- lmer(perf1 ~ exporter + size + car1 + car6 + 
                   factor(obst1) + factor(obst2) + factor(obst3) + factor(obst4) + factor(obst5) + factor(obst6) + factor(obst7) + factor(obst8) + factor(obst9) + factor(obst10) + factor(obst11) + factor(obst12) + factor(obst13) + factor(obst14) + factor(obst15) +
                   pwt_hci + fh_rol2 + p_polity2 + log(gle_cgdpc) +
                   factor(obst1):log(gle_cgdpc) +
                   factor(obst11):log(gle_cgdpc) +
                   (1 + log(gle_cgdpc) |country2), data = es2)
summary(model_0_4)

####################################################################################################

model_0_5 <- lmer(perf1 ~ factor(obst1) + factor(obst11) +
#                  exporter + size + car1 + car6 + 
#                  pwt_hci + fh_rol2 + p_polity2 + 
                   log(gle_cgdpc) +
                   factor(obst1):log(gle_cgdpc) +
                   factor(obst11):log(gle_cgdpc) +
